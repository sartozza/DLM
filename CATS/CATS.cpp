...
CATS::CATS() : NumPotPars(2), NumSourcePars(3)
{
...
void CATS::DelMomIpMp()
{
    if (RelativeMomentum)
    {
        delete[] RelativeMomentum;
        RelativeMomentum = NULL;
        delete[] RelativePosition;
        RelativePosition = NULL;
        delete[] RelativeCosTheta;
        RelativeCosTheta = NULL;
        delete[] PairMomBin;
        PairMomBin = NULL;
        delete[] PairIpBin;
...
void CATS::DelMom()
{
    if (MomBinConverged)
    {
        delete[] MomBinConverged;
        MomBinConverged = NULL;
    }
    if (LoadedPairsPerMomBin)
    {
        delete[] LoadedPairsPerMomBin;
        LoadedPairsPerMomBin = NULL;
    }
    if (kCorrFun)
    {
...
void CATS::SetQuantumStatistics(short qs)
{
    ...
    case 1:
        ...
        break;
    default:
        Identical = (pdgID[0] == pdgID[1]);
        break;
    }
    if (Identical == IdenticalParticles)
        return;
...
void CATS::SetNumChannels(const unsigned short &numCh)
{
    ...
    {
        delete[] ShortRangePotential;
    }
    if (ShortRangeComplexPotential)
    {
        delete[] ShortRangeComplexPotential;
    }
...
void CATS::SetMomBins(const unsigned &nummombins, const double &MinMom, const double &MaxMom)
{
    ...
    if (nummombins == NumMomBins && MomBin[0] == MinMom && MomBin[nummombins] == MinMom + double(nummombins) * BinWidth)
        return;
    if (nummombins != NumMomBins || !MomBin)
    {
        if (MomBin)
...
{
    if (ExcludeFailedConvergence == efb)
        return;
...
{
    if (uMomBin >= NumMomBins || uIpBin >= NumIpBins || !LoadedData)
        return 0;
...
{
    if (uMomBin >= NumMomBins || !LoadedData)
        return 0;
...
{
    if (WhichMomBin >= NumMomBins || !kCorrFun)
        return 0;
...
{
    if (Momentum < MomBin[0] || Momentum > MomBin[NumMomBins])
        return 0;
...
{
    if (WhichMomBin >= NumMomBins || !kCorrFunErr)
        return 0;
...
{
    if (WhichMomBin >= NumMomBins || WhichIpBin >= NumIpBins || !kbCorrFun)
        return 0;
...
{
    if (NumMomBins <= WhichMomBin || NumIpBins <= WhichIpBin || !kCorrFun)
        return 0;
    Momentum = WhichMomBin < NumMomBins ? MomBinCenter[WhichMomBin] : 0;
    // Momentum = WhichMomBin<NumMomBins?GetMomentum(WhichMomBin):0;
...
{
    if (NumMomBins <= WhichMomBin || NumCh <= usCh || NumPW[usCh] <= usPW || SavedWaveFunBins[WhichMomBin][usCh][usPW] <= WhichRadBin)
        return 0;
...
{
    if (NumMomBins <= uMomBin || NumCh <= usCh)
        return 0;
...
{
    if (AnalyticSource == AS && *AnaSourcePar == Pars)
        return;
    // if(!Pars){
...
{
    if (ForwardedSource == FS && AnaSourcePar->GetNumPars() == numparameters)
        return;
    delete AnaSourcePar;
    ...
    {
        if (Notifications >= nWarning)
            printf("\033[1;33mWARNING:\033[0m NULL pointer to the source function!\n");
        return;
    }
    if (!context)
    {
        if (Notifications >= nWarning)
            printf("\033[1;33mWARNING:\033[0m NULL pointer to the source context!\n");
        return;
    }
    AnalyticSource = NULL;
    ForwardedSource = FS;
...
void CATS::SetAnaSource(const unsigned &WhichPar, const double &Value, const bool &SmallChange)
{
    ...
    {
        if (Notifications >= nWarning)
            printf("\033[1;33mWARNING:\033[0m The source parameter was not set, as it exceeds the number of allowed parameters.\n");
        return;
    }
    ...
    if (AnaSourcePar)
    {
        ...
        // usleep(1e6);
        if (AnaSourcePar->GetParameter(WhichPar) == Value)
            return;
...
void CATS::SetExternalWaveFunction(const unsigned &usCh, const unsigned &usPW, DLM_Histo<complex<double>> &histWF, DLM_Histo<complex<double>> &histPS)
{
    ...
    {
        ExternalWF[usCh][usPW] = new DLM_Histo<complex<double>>();
    }
    *ExternalWF[usCh][usPW] = histWF;
    if (!ExternalPS[usCh][usPW])
    {
        ExternalPS[usCh][usPW] = new DLM_Histo<complex<double>>();
    }
...
void CATS::KillTheCat(const int &Options)
{
    ...
    if (PoorManRenorm != 0 && UseAnalyticSource == false)
        TotWaveFunEvaluated *= ComputedCorrFunction;
...
void CATS::ComputeTheRadialWaveFunction()
{
    ...
    {
        if (Notifications >= nError)
            printf("\033[1;31mERROR:\033[0m The momentum bins are not defined!\n");
        return;
    }
    ComputeWaveFunction();
}
...
void CATS::ComputeWaveFunction()
{
    ...
    for (unsigned short usCh = 0; usCh < NumCh; usCh++)
    {
        ...
        for (unsigned short usPW = 0; usPW < NumPW[usCh]; usPW++)
        {
            ...
            if (WfType[usCh][usPW] == wSchroedinger && !ShortRangePotential[usCh][usPW])
                continue;
...
void CATS::ComputeTotWaveFunction(const bool &ReallocateTotWaveFun)
{
    ...
    for (unsigned uMomBin = 0; uMomBin < NumMomBins; uMomBin++)
    {
        ...
        for (unsigned uGrid = 0; uGrid < NumGridPts; uGrid++)
        {
            if (PoorManRenorm != 1 && UseAnalyticSource == false)
                Radius = BaseSourceGrid->GetParValue(uGrid, 0) * FmToNu * PoorManRenorm;
            else
            ...
            {
                WaveFunction2[uMomBin][uGrid][usCh] = ThetaDependentSource ? EffectiveFunctionTheta(uMomBin, Radius, CosTheta, usCh) : EffectiveFunction(uMomBin, Radius, usCh);
            }
...
void CATS::ComputeComplexWaveFunction()
{
    ...
    for (unsigned uMPP = 0; uMPP < TotalNumberOfBins; uMPP++)
    {
        ...
        if (WfType[usCh][usPW] == wSchroedinger)
        {
            ...
            MinDeltaRad = sqrt(fabs(EpsilonProp / (0.5 * (PropFunVal1[0] + PropFunVal2[0]) + 1e-64)));
            MaxDeltaRad = sqrt(EpsilonProp / (Momentum * Momentum));
            if (MinDeltaRad > MaxDeltaRad)
            {
                MinDeltaRad = MaxDeltaRad;
            ...
            WaveFunR[kCurrent] = ReferencePartialWave(PosRad[kCurrent], Momentum, usPW, q1q2);
            WaveFunI[kCurrent] = ReferencePartialWave(PosRad[kCurrent], Momentum, usPW, q1q2);
            ...
            while ((!Converged || (PosRad[kCurrent] < ConvergenceRadius + ConvIntervalRadLength)) && (PosRad[kOld] < MaxRad || Rho[kOld] < MaxRho || Converged))
            {
                ...
                DeltaRad2[kNew] = EpsilonProp / (0.5 * fabs(PropFunVal1[kNew] + PropFunVal2[kNew]) + 1e-64); /// removed real and considering only modulus
                ...
                Convergence = fabs((PropFunWithoutSI[kNew] - PropFunVal1[kNew]) / (PropFunWithoutSI[kNew] + PropFunVal1[kNew] + 1.e-64)) < EpsilonConv && fabs((0. - PropFunVal2[kNew]) / (0. + PropFunVal2[kNew] + 1.e-64)) < EpsilonConv && fabs((PropFunWithoutSI[kCurrent] - PropFunVal1[kCurrent]) / (PropFunVal1[kCurrent] + PropFunWithoutSI[kCurrent] + 1.e-64)) < EpsilonConv && fabs((0. - PropFunVal2[kCurrent]) / (PropFunVal2[kCurrent] + 0. + 1.e-64)) < EpsilonConv && fabs((PropFunWithoutSI[kOld] - PropFunVal1[kOld]) / (PropFunVal1[kOld] + PropFunWithoutSI[kOld] + 1.e-64)) < EpsilonConv && fabs((0. - PropFunVal2[kOld]) / (PropFunVal2[kOld] + 0. + 1.e-64)) < EpsilonConv;
            ...
            if (MomBinConverged[uMomBin] || !ExcludeFailedConvergence)
            {
                ...
                unsigned int Point_N = StepOfMaxConvergedNumWF;
                unsigned int Point_NMin1 = Point_N - 1;

                double Rad_N = BufferRad[Point_N];
                double Rad_NMin1 = BufferRad[Point_NMin1];

                if ((!abs(BufferWaveFunction[Point_N]) || !abs(BufferWaveFunction[Point_N + 1]) && Notifications >= nWarning))
                {
                    printf("\033[1;33mWARNING:\033[0m BufferWaveFunction is zero, which is not allowed and points to a bug in the code!\n");
                    printf("         Please contact the developers and do not trust your current results!\n");
                }

                /// Numerical solution at r_N and r_N-1
                ...
                complex<double> BCoeff = (u_Rad_N * OutgoingPW_Rad_NMin1 - u_Rad_NMin1 * OutgoingPW_Rad_N) / (IncomingPW_Rad_NMin1 * OutgoingPW_Rad_N - IncomingPW_Rad_N * OutgoingPW_Rad_NMin1);

                complex<double> Asy_Rad_N = (OutgoingPW_Rad_N - (BCoeff / ACoeff) * IncomingPW_Rad_N);
                complex<double> NormSol = 1. / ACoeff;
                ...
                //     // we set up those as bin-range (i.e. the middle of the bin should be buffer rad)
                //     // we start from r=0, then proceed by adding the limit between different bins as the mid-way to the next point
                WaveFunRad[uMomBin][usCh][usPW][0] = 0;
                for (unsigned uPoint = 1; uPoint < SWFB; uPoint++)
                {
                    WaveFunRad[uMomBin][usCh][usPW][uPoint] = (BufferRad[uPoint] + BufferRad[uPoint - 1]) * 0.5;
            ...
            if (!Converged)
            {
                MomBinConverged[uMomBin] = false;
                if (Notifications >= nWarning)
                    printf("          \033[1;33mWARNING:\033[0m A momentum bin at %.2f MeV failed to converge in the complex!\n", Momentum);
                if (ExcludeFailedConvergence && Notifications >= nWarning)
                {
                    printf("                   It will be excluded from the final result!\n");
                }
            ...
            if (!NR_Status)
            {
                MomBinConverged[uMomBin] = false;
                if (Notifications >= nWarning)
                    printf("          \033[1;33mWARNING:\033[0m The failed NewtonRapson in the complex occurred at %.2f MeV!\n", Momentum);
                if (ExcludeFailedConvergence && Notifications >= nWarning)
                {
                    printf("                   The corresponding bin will be excluded from the final result in the complex!\n");
                }
            }
...
short CATS::LoadData(const unsigned short &NumBlankHeaderLines)
{
    ...
    if (!WeightIp)
        WeightIp = new double[NumIpBins];
    if (!WeightIpError)
        WeightIpError = new double[NumIpBins];
    ...
    {
        SkipThisEvent = false;
        if (NumPairs >= MaxTotPairs)
            break;
        if (NumTotalPairs >= MaxPairsToRead)
            break;
        ...
        ImpPar = fabs(ImpPar);
        if (ImpPar < IpBin[0] || ImpPar > IpBin[NumIpBins])
        {
            SkipThisEvent = true;
            WhichIpBin = 0;
        }
        ...
        TotNumEvents++;
        if (NumPartInEvent > HighMultLimit)
        ...
        for (unsigned uMomBin = 0; uMomBin < NumMomBins; uMomBin++)
        {
            for (unsigned uIpBin = 0; uIpBin < NumIpBins; uIpBin++)
            {
                ...
                if (pTemp < pMaxPairsPerBin)
                    pMaxPairsPerBin = pTemp;
            }
        }
...
unsigned CATS::LoadDataBuffer(const unsigned &WhichIpBin, CatsDataBuffer *KittyBuffer)
{
    ...
    for (unsigned uPair = 0; uPair < KittyBuffer->GetNumPairs(); uPair++)
    {
        ...
        if (UseTotMomCut)
            TotalPairMomentum[NumPairs] = PairSum->GetP() * 1000;
        ...
        for (unsigned uIpBin = 0; uIpBin < NumIpBins; uIpBin++)
        {
            ...
            for (unsigned uGrid = 0; uGrid < NumGridPts; uGrid++)
            {
                ...
                if (!SourceVal)
                    continue;
                SourceInt += SourceVal;
                if (Radius < SourceMinRad || Radius > SourceMaxRad)
                {
                    continue;
                }
                ...
                WaveFunVal = 0;
                for (unsigned usCh = 0; usCh < NumCh; usCh++)
                    WaveFunVal += (WaveFunction2[uMomBin][uGrid][usCh]) * ChannelWeight[usCh];
...
void CATS::SortAllData()
{
    ...
    if (UseTotMomCut)
        ResortData(TotalPairMomentum, SortTool);
...
{
    if (BaseSourceGrid)
        BaseSourceGrid->Update();
    for (unsigned uMomBin = 0; uMomBin < NumMomBins; uMomBin++)
    {
        if (kSourceGrid && kSourceGrid[uMomBin])
            kSourceGrid[uMomBin]->Update();
    }
    if (kbSourceGrid)
    {
        for (unsigned uMomBin = 0; uMomBin < NumMomBins; uMomBin++)
                                      ...
                                      const unsigned short &usPW, const unsigned short &usCh)
{
    ...
    //! In principle this should be executed only if ShortRangePotential[usCh][usPW] is defined.
    //! Do note that this function is NEVER called in case this is not true! Make sure that this stays so!
    /// !VERY IMPORTANT: SINCE WE AIM AT SOLVING THE SE IN THE FEMTOSCOPIC APPROACH, WE NEED TO USE THE HERMITE CONJUGATE OF THE COMPLEX POTENTIAL. This change has to be done internally, so that the user can still call out the original potential stored in ShortRangeComplexPotential
    ///! Care will have to be taken when implementing for two channels.
    double RealPot = real(ShortRangeComplexPotential[usCh][usPW](Parameters));
    double ImagPot = (-1)*imag(ShortRangeComplexPotential[usCh][usPW](Parameters));
...
double CATS::BesselFunction(const double &Radius, const double &Momentum, const unsigned short &usPW) const
{
    ...
    // N.B. gsl_sf_bessel_jl are defined for Rho>0, but in principle the bessel functions are symmetric for even l
    // and anti-symmetric for odd l, this is implemented here.
    return Rho > 0 ? gsl_sf_bessel_jl(usPW, Rho) : pow(-1, usPW) * gsl_sf_bessel_jl(usPW, -Rho);
}
...
double CATS::NeumannFunction(const double &Radius, const double &Momentum, const unsigned short &usPW) const
{
    ...
    // N.B. gsl_sf_bessel_jl are defined for Rho>0, but in principle the bessel functions are symmetric for even l
    // and anti-symmetric for odd l, this is implemented here.
    return Rho > 0 ? gsl_sf_bessel_yl(usPW, Rho) : (-1) * pow(-1, usPW) * gsl_sf_bessel_yl(usPW, -Rho);
}
/// u^+ =e^(+ikr)/(2ik) based on the math starting from the decomposition in terms of Bessel and Neumann
complex<double> CATS::OutgoingPlanePartialWave(const double &Radius, const double &Momentum, const unsigned short &usPW) const
{
    double Rho = Radius * Momentum;
    // if Rho is zero, the gsl function Spherical Bessel/Neumann will not work
    if (!Rho)
    {
        return 0;
    }
...
/// u^- =e^(-ikr)/(2ik) based on the math starting from the decomposition in terms of Bessel and Neumann
complex<double> CATS::IncomingPlanePartialWave(const double &Radius, const double &Momentum, const unsigned short &usPW) const
{
    double Rho = Radius * Momentum;
    // if Rho is zero, the gsl function Irregular Spherical Bessel will not work
    if (!Rho)
                          ...
                          const double &xMin, const double &xMax, const double &fValShift, bool &status) const
{
    ...
    for (unsigned iIter = 0; iIter < maxIter; iIter++)
    {
        ...
        while (((fabs(fValNew) > fabs(fVal) && counter < 16) || (xVal < xMin || xVal > xMax)))
        {
            counter++;
...
template <class Type>
void CATS::ResortData(Type *input, DLM_Sort<int64_t, unsigned> &Sorter)
{
                                        ...
                                        const unsigned short &usCh, const unsigned short &usPW, const bool &DivideByR, const bool &Asymptotic) const
{
    ...
    /// This is filled in the normalization, so this is why we are not entering in the first loop
    unsigned &SWFB = SavedWaveFunBins[uMomBin][usCh][usPW];
...
double CATS::EffectiveFunction(const unsigned &uMomBin, const double &Radius, const unsigned short &usCh)
{
    ...
    for (unsigned short usPW = 0; usPW < MaxPw; usPW++)
    {
        ...
        else if (!OnlyNumPw[usCh])
        {
            ...
            if (usPW >= NumPW[usCh] && abs(OldResult) < 1e-7 && abs(Result) < 1e-8)
                break;
            OldResult = Result;
        }
    }
...
double CATS::EffectiveFunctionTheta(const unsigned &uMomBin, const double &Radius, const double &CosTheta, const unsigned short &usCh)
{
    ...
    for (unsigned short usPW = 0; usPW < MaxPw; usPW++)
    {
        ...
        {
            if (LegPol[usPW] == 1e6)
                LegPol[usPW] = gsl_sf_legendre_Pl(usPW, CosTheta);
            if (SolvedPartWave[usPW] == 1e6)
                SolvedPartWave[usPW] = EvalWaveFunctionU(uMomBin, Radius, usCh, usPW, true);
        ...
        else if (!OnlyNumPw[usCh])
        {
            if (LegPol[usPW] == 1e6)
                LegPol[usPW] = gsl_sf_legendre_Pl(usPW, CosTheta);
            if (RefPartWave[usPW] == 1e6)
                RefPartWave[usPW] = ReferencePartialWave(Radius, Momentum, usPW, Q1Q2);
            ...
            if (usPW >= NumPW[usCh] && abs(OldResult) < 3.16e-4 && abs(Result) < 1e-4)
                break;
            OldResult = Result;
        }
        else
...
template <class Type>
Type CATS::GetBinCenter(const Type *Bins, const unsigned &WhichBin) const
{
    return 0.5 * (Bins[WhichBin] + Bins[WhichBin + 1]);
}

template <class Type>
Type CATS::EvalBinnedFun(const double &xVal, const unsigned &NumBins, const double *Bins, const double *BinCent, const Type *Function) const
{
    if (xVal < Bins[0] || xVal > Bins[NumBins])
        return 0;
    if (NumBins == 1)
        return Function[0];

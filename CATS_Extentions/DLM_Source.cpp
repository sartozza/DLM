...
double PoissonSum(const double &xVal, const KdpPars &kdppars)
{
    ...
    for (unsigned uP = 0; uP < KdpPars::NumDistos; uP++)
    {
        ...
        if (Std2)
            Pssn = DLM_Poisson(xVal * Mean / Std2, Mean * Mean / Std2) * Mean / Std2;
        else
            Pssn = 0;
        Rslt += Norm * Pssn;
        if (RemainingNorm == Norm)
            RemainingNorm = 0;
        else
            RemainingNorm -= Norm;
...
double Gauss_Exp_Approx(double *Pars)
{
    ...
    if (MASS <= 0)
    {
        printf("\033[1;33mWARNING:\033[0m Gauss_Exp_Approx got MASS=0\n");
        return 0;
    }
...
double Gauss_Exp_Exact(double *Pars)
{
    ...
    else if (TKM1 == 0)
    {
        return (pow(E, -pow(RAD, 2) / (4. * pow(SIG, 2)) - RAD / TKM2) * (4 * pow(E, RAD / TKM2) * SIG * TKM2 * (pow(SIG, 2) + pow(TKM2, 2)) -
                                                                          ...
                                                                          pow(E, pow(RAD, 2) / (4. * pow(SIG, 2)) + pow(SIG, 2) / pow(TKM2, 2)) * sqrt(Pi) * (-4 * pow(SIG, 4) + 2 * pow(SIG, 2) * (RAD - 3 * TKM2) * TKM2 + RAD * pow(TKM2, 3)) *
                                                                              (erf(RAD / (2. * SIG) - SIG / TKM2) + erf(SIG / TKM2)))) /
               (sqrt(Pi) * pow(TKM2, 5));
    }
    else if (TKM2 == 0)
    {
        return (pow(E, -pow(RAD, 2) / (4. * pow(SIG, 2)) - RAD / TKM1) * (4 * pow(E, RAD / TKM1) * SIG * TKM1 * (pow(SIG, 2) + pow(TKM1, 2)) -
                                                                          ...
                                                                          pow(E, pow(RAD, 2) / (4. * pow(SIG, 2)) + pow(SIG, 2) / pow(TKM1, 2)) * sqrt(Pi) * (-4 * pow(SIG, 4) + 2 * pow(SIG, 2) * (RAD - 3 * TKM1) * TKM1 + RAD * pow(TKM1, 3)) *
                                                                              (erf(RAD / (2. * SIG) - SIG / TKM1) + erf(SIG / TKM1)))) /
               (sqrt(Pi) * pow(TKM1, 5));
    }
    ...
    else if (TKM1 == TKM2)
    {
        ...
        if (WARNING)
            printf("WARNING: shouldn't I take TKM=TKM1+TKM2 for the computation, verify!\n");
        WARNING = false;
        return (pow(E, (pow(SIG, 2) - RAD * TKM1) / pow(TKM1, 2)) * ((-2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM1, 2)) * TKM1) + (4 * (-1 + pow(E, -pow(SIG, 2) / pow(TKM1, 2))) * pow(SIG, 3)) / TKM1 -
                                                                     ...
                                                                     (2 * sqrt(Pi) * pow(SIG, 4) * erf(SIG / TKM1)) / pow(TKM1, 2) - (sqrt(Pi) * pow(SIG, 3) * (2 * pow(SIG, 2) - RAD * TKM1) * erf(abs(-2 * pow(SIG, 2) + RAD * TKM1) / (2. * SIG * TKM1))) / (pow(TKM1, 3) * abs(-RAD / (2. * SIG) + SIG / TKM1)) - (pow(2 * pow(SIG, 2) - RAD * TKM1, 3) * (-abs(-2 * pow(SIG, 2) + RAD * TKM1) + pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM1, 2) / (4. * pow(SIG, 2) * pow(TKM1, 2))) * sqrt(Pi) * SIG * TKM1 * erf(abs(-2 * pow(SIG, 2) + RAD * TKM1) / (2. * SIG * TKM1)))) / (8. * pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM1, 2) / (4. * pow(SIG, 2) * pow(TKM1, 2))) * pow(SIG, 2) * pow(TKM1, 4) * pow(abs(-RAD / (2. * SIG) + SIG / TKM1), 3)))) /
               (sqrt(Pi) * pow(SIG, 2) * TKM1);
    }
    else
    {
        return (pow(E, (pow(SIG, 2) - RAD * TKM1) / pow(TKM1, 2)) * ((-2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM1, 2)) * TKM1) + (4 * (-1 + pow(E, -pow(SIG, 2) / pow(TKM1, 2))) * pow(SIG, 3)) / TKM1 -
                ...
                pow(E, (pow(SIG, 2) - RAD * TKM2) / pow(TKM2, 2)) * ((2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM2, 2)) * TKM2) - (4 * (-1 + pow(E, -pow(SIG, 2) / pow(TKM2, 2))) * pow(SIG, 3)) / TKM2 +
                                                                     ...
                                                                     (2 * sqrt(Pi) * pow(SIG, 4) * erf(SIG / TKM2)) / pow(TKM2, 2) + (sqrt(Pi) * pow(SIG, 3) * (2 * pow(SIG, 2) - RAD * TKM2) * erf(abs(-2 * pow(SIG, 2) + RAD * TKM2) / (2. * SIG * TKM2))) / (pow(TKM2, 3) * abs(RAD / (2. * SIG) - SIG / TKM2)) + (pow(2 * pow(SIG, 2) - RAD * TKM2, 3) * (-abs(-2 * pow(SIG, 2) + RAD * TKM2) + pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM2, 2) / (4. * pow(SIG, 2) * pow(TKM2, 2))) * sqrt(Pi) * SIG * TKM2 * erf(abs(-2 * pow(SIG, 2) + RAD * TKM2) / (2. * SIG * TKM2)))) / (8. * pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM2, 2) / (4. * pow(SIG, 2) * pow(TKM2, 2))) * pow(SIG, 2) * pow(TKM2, 4) * pow(abs(RAD / (2. * SIG) - SIG / TKM2), 3)))) /
               (sqrt(Pi) * pow(SIG, 2) * (TKM1 - TKM2));
    }
}
...
double GaussExpSimple_Exact(double *Pars)
{
    ...
    else
    {
        return (pow(E, (pow(SIG, 2) - RAD * TKM) / pow(TKM, 2)) * ((-2 * pow(SIG, 3)) / (pow(E, pow(SIG, 2) / pow(TKM, 2)) * TKM) + (4 * (-1 + pow(E, -pow(SIG, 2) / pow(TKM, 2))) * pow(SIG, 3)) / TKM -
                                                                   ...
                                                                   (2 * sqrt(Pi) * pow(SIG, 4) * erf(SIG / TKM)) / pow(TKM, 2) - (sqrt(Pi) * pow(SIG, 3) * (2 * pow(SIG, 2) - RAD * TKM) * erf(abs(-2 * pow(SIG, 2) + RAD * TKM) / (2. * SIG * TKM))) / (pow(TKM, 3) * abs(-RAD / (2. * SIG) + SIG / TKM)) - (pow(2 * pow(SIG, 2) - RAD * TKM, 3) * (-abs(-2 * pow(SIG, 2) + RAD * TKM) + pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM, 2) / (4. * pow(SIG, 2) * pow(TKM, 2))) * sqrt(Pi) * SIG * TKM * erf(abs(-2 * pow(SIG, 2) + RAD * TKM) / (2. * SIG * TKM)))) / (8. * pow(E, pow(-2 * pow(SIG, 2) + RAD * TKM, 2) / (4. * pow(SIG, 2) * pow(TKM, 2))) * pow(SIG, 2) * pow(TKM, 4) * pow(abs(-RAD / (2. * SIG) + SIG / TKM), 3)))) /
               (sqrt(Pi) * pow(SIG, 2) * TKM);
    }
}
...
double GaussExpTotSimple(double *Pars)
{
    ...
    double Result = 0;
    TKMA = 0;
    Result += primA * primB * GaussExpSimple(Pars); // both primary
    TKMA = oldTKMB;
    Result += primA * (1. - primB) * GaussExpSimple(Pars); // B comes from resonance
    TKMA = oldTKMA;
    Result += (1. - primA) * primB * GaussExpSimple(Pars); // A comes from resonance
    TKMA = oldTKMA + oldTKMB;
    Result += (1. - primA) * (1. - primB) * GaussExpSimple(Pars); // both come from resonance
...
double GaussExpTotIdenticalSimple(double *Pars)
{
    ...
    double Result = 0;
    TKM = 0;
    Result += prim * prim * GaussExpSimple(Pars); // both primary
    // printf("Result = %.2e; GES=%.2e*%.2e\n",Result,prim*prim,GaussExpSimple(Pars));
    TKM = oldTKM;
    Result += 2 * prim * (1. - prim) * GaussExpSimple(Pars); // 1 comes from resonance
    // printf(" Result = %.2e; GES=%.2e*%.2e\n",Result,2*prim*(1.-prim),GaussExpSimple(Pars));
    TKM = 2 * oldTKM;
    Result += (1. - prim) * (1. - prim) * GaussExpSimple(Pars); // both come from resonance
...
void MS_GaussExp_mT_Simple::SetNum_mT(const unsigned &nmt)
{
    ...
    if (Mean_mT)
        delete[] Mean_mT;
    Mean_mT = new double[nmt];
    if (Weight_mT)
        delete[] Weight_mT;
    Weight_mT = new double[nmt];
    if (FunctionValue)
        delete[] FunctionValue;
    FunctionValue = NULL;
}
...
{
    if (NumPtsStability == numPts && MinStability == minVal && MaxStability == maxVal)
        return;
    Reset();
...
{
    if (NumPtsScale == numPts && MinScale == minVal && MaxScale == maxVal)
        return;
    Reset();
    NumPtsScale = numPts;
    MinScale = minVal;
...
{
    if (NumPtsRad == numPts && MinRad == minVal && MaxRad == maxVal)
        return;
    Reset();
    NumPtsRad = numPts;
    MinRad = minVal;
...
#pragma omp critical
                    {
                        ...
                        if (Type == 0)
                        {
                            Histo->SetBinContent(WhichBin, LevySource3D_single(BIN_PARS));
                        }
                        else if (Type == 1)
                        {
                        ...
                        {
                            Histo->SetBinContent(WhichBin, LevySource3D(BIN_PARS));
                        }
                    }
                }
            }
...
void DLM_CleverLevy::Init()
{
    ...
    if (NumPtsRad == 1)
    {
        Histo->SetUp(0, NumPtsRad, MinRad, MaxRad);
    }
    else
    {
    ...
    if (NumPtsScale == 1)
    {
        Histo->SetUp(1, NumPtsScale, MinScale, MaxScale);
    }
    else
    {
    ...
    if (NumPtsStability == 1)
    {
        Histo->SetUp(2, NumPtsStability, MinStability, MaxStability);
    }
    else
    {
...
void DLM_CleverMcLevyReso::InitReso(const unsigned &whichparticle, const unsigned &numreso)
{
    if (whichparticle >= 2)
    {
        printf("\033[1;33mWARNING:\033[0m You can call InitReso only for particle 0 or 1\n");
        return;
    }
    if (NumResonances[whichparticle] == numreso)
        return;
    NumResonances[whichparticle] = numreso;
    if (ResoWeight[whichparticle])
    {
        delete[] ResoWeight[whichparticle];
        ResoWeight[whichparticle] = new double[NumResonances[whichparticle]];
    }
    if (ResoMass[whichparticle])
    {
        delete[] ResoMass[whichparticle];
        ResoMass[whichparticle] = new double[NumResonances[whichparticle]];
    }
    if (ResoTau[whichparticle])
    {
        delete[] ResoTau[whichparticle];
        ResoTau[whichparticle] = new double[NumResonances[whichparticle]];
    }
    if (ChildMass0[whichparticle])
    {
        delete[] ChildMass0[whichparticle];
        ChildMass0[whichparticle] = new double[NumResonances[whichparticle]];
    }
    if (ChildMass1[whichparticle])
    {
        delete[] ChildMass1[whichparticle];
        ChildMass1[whichparticle] = new double[NumResonances[whichparticle]];
    }
    if (SmearResoMomentum[whichparticle])
    {
        delete[] SmearResoMomentum[whichparticle];
        SmearResoMomentum[whichparticle] = new double[NumResonances[whichparticle]];
    }
    if (SmearResoMass[whichparticle])
    {
        delete[] SmearResoMass[whichparticle];
        SmearResoMass[whichparticle] = new bool[NumResonances[whichparticle]];
    }
    if (ResoDecayTopology[whichparticle])
    {
        delete[] ResoDecayTopology[whichparticle];
        ResoDecayTopology[whichparticle] = new RESO_DEC_TOP[NumResonances[whichparticle]];
    }
    if (ResoEmissionAngle[whichparticle])
    {
        delete[] ResoEmissionAngle[whichparticle];
        ResoEmissionAngle[whichparticle] = new const DLM_Histo<double> *[NumResonances[whichparticle]];
    }
    if (Histo)
        Histo->SetBinContentAll(1e6);
}
void DLM_CleverMcLevyReso::SetUpReso(const unsigned &whichparticle, const unsigned &whichreso, const double &weight, const double &mass, const double &tau, const double &mass0, const double &mass1, const double &momSmear, const bool &massSmear, const RESO_DEC_TOP &rdt)
{
    if (whichparticle >= 2)
    {
        printf("\033[1;33mWARNING:\033[0m You can call SetUpReso only for particle 0 or 1\n");
        return;
    }
    if (whichreso >= NumResonances[whichparticle])
    {
        printf("\033[1;33mWARNING:\033[0m Only %u number of resonances are currently allowed. Change using InitReso\n", NumResonances[whichparticle]);
        return;
    }
    if (!ResoWeight[whichparticle])
    {
...
void DLM_CleverMcLevyReso::SetUpResoEmission(const unsigned &whichparticle, const unsigned &whichreso, const DLM_Histo<double> *Distr)
{
    if (whichparticle >= 2)
    {
        printf("\033[1;33mWARNING:\033[0m You can call SetUpResoEmission only for particle 0 or 1\n");
        return;
    }
    if (whichreso >= NumResonances[whichparticle])
    {
...
#pragma omp critical
                ...
                for (unsigned uIter = 0; uIter < NumMcIter; uIter++)
                {
                    ...
                    if (Type == 0)
                    {
                        RAD = RanGen.StableR(3, par_stability, 0, par_scale, 0);
                    }
                    else if (Type == 1)
                    {
                        RAD = RanGen.StableDiffR(3, par_stability, 0, par_scale, 0);
                    }
                    else
                    {
                        RAD = RanGen.StableNolan(3, par_stability, 0, par_scale, 0);
                    }
                    // OLDRAD = RAD;
                    // printf("par_scale=%f\n",par_scale);
                    ...
                    for (unsigned uParticle = 0; uParticle < 2; uParticle++)
                    {
                        if (!ResoWeight || !ResoWeight[uParticle])
                            continue;
                        ...
                        ResoMomentum = sqrt(pow(SmearedResoMass, 4.) - 2. * pow(SmearedResoMass * ChildMass0[uParticle][WhichReso], 2.) +
                                            ...
                                            pow(ChildMass1[uParticle][WhichReso], 4.)) /
                                       (2. * ChildMass0[uParticle][WhichReso]);
                        ...
                        case rdtRandom:
                            if (ResoEmissionAngle && ResoEmissionAngle[uParticle] && ResoEmissionAngle[uParticle][WhichReso])
                            {
                                ...
                                // make sure that the direction is inverted for the second particle
                                if (uParticle)
                                    ResoPathSpherical[uParticle][1] = fabs(ResoPathSpherical[uParticle][1] - Pi);
...
double DLM_CleverMcLevyResoTM::Eval(double *Pars)
{
    ...
    {
        return -1;
        printf("Possible problem in DLM_CleverMcLevyResoTM::Eval\n");
    }
    double &Radius = Pars[1];
    double &Scale = Pars[3];
    double &Stability = Pars[4];
    ...
    int ScaleBin = Histo->GetBin(1, Scale);
    int StabilityBin = Histo->GetBin(2, Stability);
    if (ScaleBin < 0 || ScaleBin >= int(Histo->GetNbins()))
    {
        printf("\033[1;33mWARNING!\033[0m A bad scaling parameter passed into DLM_CleverMcLevyResoTM::Eval\n");
        return 0;
    }
    if (StabilityBin < 0 || StabilityBin >= int(Histo->GetNbins()))
    {
        printf("\033[1;33mWARNING!\033[0m A bad stability parameter passed into DLM_CleverMcLevyResoTM::Eval\n");
        return 0;
    }
    unsigned WhichBin[3];
    double par_stability;
...
#pragma omp critical
                ...
                for (unsigned uIter = 0; uIter < NumMcIter; uIter++)
                {
                    ...
                    RAD = sqrt(RAD * RAD + BGT[0] * BGT[0] + BGT[1] * BGT[1] - 2. * RAD * BGT[0] * CosRcP0 + 2. * RAD * BGT[1] * CosRcP1 - 2. * BGT[0] * BGT[1] * CosP0P1);
                    // static double RAD_avg=0;
                    // RAD_avg+=RAD;
...
DLM_CecaSource_v0::DLM_CecaSource_v0(const std::string systype, const std::string anaver, const std::string infolder) : SystemType(systype), AnaVersion(anaver), InputFolder(infolder)
{
    ErrorState = false;
    dlmSource = NULL;
    AnaVersionBase = "";
            ...
            while (!feof(InFile))
            {
                ...
                if (ReadMode == 0)
                {
                    ...
                    else
                    {
                        ...
                        if (strcmp(cdscr, "d_x") == 0)
                        {
                            d_x = read_value;
                        }
                        if (strcmp(cdscr, "d_y") == 0)
                        {
                            d_y = read_value;
                        }
                        if (strcmp(cdscr, "d_z") == 0)
                        {
                            d_z = read_value;
                        }
                        if (strcmp(cdscr, "h_x") == 0)
                        {
                            h_x = read_value;
                        }
                        if (strcmp(cdscr, "h_y") == 0)
                        {
                            h_y = read_value;
                        }
                        if (strcmp(cdscr, "h_z") == 0)
                        {
                            h_z = read_value;
                        }
                        // else if(strcmp(cdscr,"h_fct")==0) {h_fct = read_value;}
                        if (strcmp(cdscr, "tau") == 0)
                        {
                            tau = read_value;
                        }
                        ...
                        if (strcmp(cdscr, "del_proton") == 0)
                        {
                            del_proton = read_value;
                        }
                        if (strcmp(cdscr, "del_proton_reso") == 0)
                        {
                            del_proton_reso = read_value;
                        }
                        if (strcmp(cdscr, "del_lambda") == 0)
                        {
                            del_lambda = read_value;
                        }
                        if (strcmp(cdscr, "del_lambda_reso") == 0)
                        {
                            del_lambda_reso = read_value;
                        }
                    } // strcmp(cdscr,"type")
                } // ReadMode==1
                else
...
bool DLM_CecaSource_v0::InitHisto()
{
    ...
    if (AnaVersion.rfind("Cigar2_ds", 0) == 0 || AnaVersion.rfind("Jaime1_ds", 0) == 0 || AnaVersion.rfind("JaimeDelay1_dLs", 0) == 0)
    { // pos=0 limits the search to the prefix
        if (AnaVersion.rfind("Cigar2_ds", 0) == 0)
            AnaVersionBase = "Cigar2";
        else if (AnaVersion.rfind("Jaime1_ds", 0) == 0)
            AnaVersionBase = "Jaime1";
        else if (AnaVersion.rfind("JaimeDelay1_dLs", 0) == 0)
            AnaVersionBase = "JaimeDelay1";
        ...
        else if (AnaVersionBase == "JaimeDelay1")
        {
            ...
            if (AnaVersionBase == "Cigar2")
                ...
                dlmSource->SetUp(3, NumHz, 2.55, 2.79);
        }
...
double DLM_CecaSource_v0::RootEval(double *x, double *pars)
{
    ...
    {
        if (SrcPars.wght[uP] < 0)
            SrcPars.wght[uP] = 0;
        if (SrcPars.wght[uP] > 1)
            SrcPars.wght[uP] = 1;
    }
    // printf("XXX\n");
    // SrcPars.Print();
...
{
    if (!dlmSource || uP >= dlmSource->GetDim())
        return 0;
    if (bincenter)
        return dlmSource->GetBinCenter(uP, 0);
    else
        return dlmSource->GetLowEdge(uP);
}
double DLM_CecaSource_v0::Up_par(unsigned uP, bool bincenter)
{
    if (!dlmSource || uP >= dlmSource->GetDim())
        return 0;
    if (bincenter)
        return dlmSource->GetBinCenter(uP, dlmSource->GetNbins(uP) - 1);
    else
        return dlmSource->GetUpEdge(uP);
}
...
DLM_MultiKdpSource::DLM_MultiKdpSource(DLM_Histo<KdpPars> &kdp_source, const int WhereIsKstar, const int WhereIsCosTheta) : inlcude_kstar(WhereIsKstar), include_costh(WhereIsCosTheta)
{
    kdp_histogram = NULL;
...
double DLM_MultiKdpSource::Eval(double *kxc)
{
    ...
    double &costh = kxc[2];
    if (!eval_at)
        eval_at = new double[kdp_histogram->GetDim()];
    if (inlcude_kstar >= 0)
    {
